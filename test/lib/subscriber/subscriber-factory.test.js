...
const Subscriber = require('../../../lib/subscriber/subscriber-factory');

describe('On Subscriber-factory module', function() {
  const channel = {
  ...
  describe('the factory function', function() {
    ...
    it('should create a Listener object with a `listen` method', function() {
      var listener = Subscriber(seneca, options);
  ...
  describe('the handleMessage() function', function() {
    ...
    const options = {
      exchange: DEFAULT_OPTIONS.exchange.name,
      queue: 'seneca.role:create',
      ch: channel,
      options: DEFAULT_OPTIONS
    };
    ...
    it('should ACK with response from a Seneca act', function(done) {
      var reply = { foo: 'bar' };
      ...
      listener
        ...
        .then(() => {
          ...
          chai.assert.isTrue(channel.sendToQueue.should.not.have.been.called);
    ...
    it('should ACK with false response from a Seneca act', function(done) {
